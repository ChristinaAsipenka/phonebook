...
if(isset($_GET['id'])){
	...
	$id = (int)$_GET['id'];
	// echo $id;
	$res_otd =  mysqli_fetch_object(mysqli_query($connect, "SELECT * FROM spr_otdel WHERE id = $id"));
	$res_podrazdelenie =  mysqli_fetch_object(mysqli_query($connect, "SELECT * FROM spr_podrazdelenie WHERE id=$res_otd->cod_podch"));
	$res_branch =  mysqli_fetch_object(mysqli_query($connect, "SELECT * FROM spr_branch WHERE id=$res_otd->cod_branch"));
	$res_users = mysqli_query($connect, "SELECT * FROM `users` WHERE spr_cod_otd = $res_otd->id and is_spr = 1 ORDER BY filter_spr");
}
...
?>
	...
	<?php //---------------------- Наполнение синей полосы----------------------
					if ($res_branch->id==7) { //для АУ
						...
						if ($const_adress_branch!=$res_otd->adress) { echo "<p class='info_adress'>$res_otd->adress<p>" ; }; //вывод адреса только если он не равен адерсу-константы1
						
					} else { //для филиалов
						echo "<h4>$res_otd->name_otdel</h4>";
						if ($const_adress_podrazd!=$res_otd->adress) { echo "<p class='info_adress'>$res_otd->adress<p>" ; }; //вывод адреса только если он не равен адерсу-константы2

					}
					if (!empty($res_otd->email) and !empty($res_otd->fax)) 
							{ echo "<p class='info_adress'>e-mail: $res_otd->email, факс: +375($res_otd->phone_cod)$res_otd->fax<p>" ; } //вывод email и fax если они не пустые
						elseif (!empty($res_otd->email) and empty($res_otd->fax)) 
							{ echo "<p class='info_adress'>e-mail: $res_otd->email<p>" ; } //вывод только email если он не пустой, а факс пустой
						elseif (empty($res_otd->email) and !empty($res_otd->fax)) 
							{ echo "<p class='info_adress'>факс: +375($res_otd->phone_cod)$res_otd->fax<p>" ; } ;//вывод только fax если он не пустой, а мыло пустое	
				?>
	</div>
...
?>
		<div class='person_data'>
			<div class='block_link'><a href='person.php?id=<?php echo $row_user->id; ?>' class='person_link'><p class='name'><span class='fam'><?php echo $row_user->fam.' ' ?></span><span class='name'><?php echo $row_user->name.' '?></span><span class='otch'><?php echo $row_user->otch; ?></p></a></div>
