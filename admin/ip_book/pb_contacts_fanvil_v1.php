...
//для Fanvil------------------------------------------------------------------------------------------------------------------------
	...
	//Запрос в таблицу users с правильной сортировкой
		$arr_users2=mysqli_query($link,"SELECT users.name, users.fam, users.otch, users.dolgnost, users.ip_phone, users.spr_cod_branch, users.ip_phone_otd, ip_group.name AS grName, ip_group.sort AS grSort, users.filter_spr
										FROM `users` 
		...
		while ($row = mysqli_fetch_array($arr_users2)){
			...
			$xx=$row['spr_cod_branch'];
			switch ($xx) {
				case 1: $spr_cod_branch="Брест";
				break;
				case 2: $spr_cod_branch="Витебск";
				break;
				case 3:	$spr_cod_branch="Гомель";
				break;
				case 4:	$spr_cod_branch="Гродно";			
				break;
				case 5:	$spr_cod_branch="Могилев";
						$row['spr_cod_branch']=6;
				break;
				case 6:	$spr_cod_branch="Минск";
						$row['spr_cod_branch']=5;
				break;
				case 7: $spr_cod_branch="Управление"; 
						$row['spr_cod_branch']=0;
				break;
			}
			...
			//$dolgn=mb_strtolower(mb_substr($row['dolgnost'],0,5),'UTF-8'); //должность в нижний регистр и обрезка до 5 букв
			$dolgn=mb_strtolower($row['dolgnost'],'UTF-8'); //должность в нижний регистр 
			$znak="-";
			if ($row['ip_phone_otd']==14 or $row['ip_phone_otd']==15 or $row['ip_phone_otd']==21){	//игнорируем 14,15,21 группы там нет главных
				}else {
					if ($zz<>$row['ip_phone_otd']){
			...
			//Вывод результата
				$rez2=$rez2.$DE1.$N1.$row['grSort'].$znak.$row['fam']." ".$row['name']." ".$row['otch']."/".$dolgn.$N2.$T1.$row['ip_phone'].$T2.$M12.$O12.$R1."Default".$R2.$G1.$row['spr_cod_branch']." ".$spr_cod_branch.$G2.$DE2;
				$total2=$total2+1;
