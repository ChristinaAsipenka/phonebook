...
//для Yealink-------------------------------------------------------------------------------------------------------	
	...
	//Запрос во вторую таблицу users с правильной сортировкой
		$arr_users=mysqli_query($link,"SELECT
		users.name, users.fam, users.otch, users.ip_phone, users.spr_cod_branch, users.ip_phone_otd, users.filter_spr, spr_podrazdelenie.sokr_name AS sokrNP, spr_otdel.sokr_name AS sokrNO 
		FROM `users` 
		...
		ORDER BY spr_cod_branch, ip_phone_otd, filter_spr"); //$arr_users=mysqli_query($link,"SELECT * FROM `users` WHERE id like '%5'");
		...
		while ($row = mysqli_fetch_array($arr_users)){
			...
			//присвоение индекса МРО и РЭГИ
			if ($row['ip_phone_otd']==14 or $row['ip_phone_otd']==15 ){	//для групп 14,15
					...
					$row['sokrNP']=$perv.mb_substr($row['sokrNP'],0,1)."_"; 						//формируем название
				...
				$spr_cod_branch=$spr_cod_branch."_".$row['sokrNP'].$row['sokrNO'];
			}
...
//для Fanvil------------------------------------------------------------------------------------------------------------------------
	...
	//Запрос в таблицу users с правильной сортировкой
		$arr_users2=mysqli_query($link,"SELECT users.name, users.fam, users.otch, users.ip_phone, users.spr_cod_branch, users.ip_phone_otd, ip_group.name AS grName, ip_group.sort AS grSort, users.filter_spr, 
											spr_podrazdelenie.sokr_name AS sokrNP, spr_otdel.sokr_name AS sokrNO
										FROM `users` 
		...
		while ($row = mysqli_fetch_array($arr_users2)){
			...
			if ($row['ip_phone_otd']==0){	//всем у кого не выбрана группа назначаем группу 21.Прочие 
				$row['ip_phone_otd']=21;
				$row['grSort']=22;
				$row['grName']="Прочие";
			}
