<?php // счетчик статистики
	$first_url="phonebook_branch-list.php";
...
?>
<div class = "block_branch_inner sticky_body">
<?php
...
?>
		...
		<div class = "block_podrazd_inner sticky_body">
			<?php //---------------------- Наполнение золотой полосы----------------------
				if ($res_branch->id==7) { //для АУ
					echo "<h3>$row_podrazd->name_podrazd</h3>";
					if ($const_adress_branch!=$row_podrazd->adress) { echo "<p class='info_adress'>$row_podrazd->adress<p>" ; } //вывод адреса только если он не равен адерсу-константы1
				} else { //для филиалов
					echo "<h3>$row_podrazd->name_podrazd</h3>";
					echo "<p class='info_adress'>$row_podrazd->adress<p>"; //вывод адреса всегда
				}
				$const_adress_podrazd=$row_podrazd->adress; //адрес-константа уровня podrazd 
			?>
		</div>
		<?php 
		...
		while($row_otd = mysqli_fetch_object($res_otd)){ ?>
			...
			<div class = "block_otd_inner sticky_body">
		...
		while($row_user = mysqli_fetch_object($res_users)){	?>
			<div class='person_data'>
				...
				<div class='add_block_link'><a href='branch-list.php?id=<?php echo $res_branch->id;?>'><?php echo $res_branch->sokr_name; ?></a><a href='department.php?id=<?php echo $row_podrazd->id;?>'><?php echo $row_podrazd->sokr_name; ?></a><a href='district.php?id=<?php echo $row_otd->id; ?>'><?php echo $row_otd->sokr_name; ?></a></div>
			</div>
